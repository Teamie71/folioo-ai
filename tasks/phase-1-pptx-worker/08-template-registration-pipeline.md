...
- 1.04 (샌드박스/렌더/GCS) — `build_meta.py` 가 soffice→PDF→pdftoppm 슬라이드 JPG·그리드 썸네일 생성에 04 의 렌더 도구 래퍼를 재사용한다(중복 구현 회피). ⚠ 모듈 공유 의존: 08 은 런타임 워커와 분리된 오프라인 도구지만 렌더 도구 체인이 04 와 겹쳐 순차 처리로 둔다(사용자 확정).
