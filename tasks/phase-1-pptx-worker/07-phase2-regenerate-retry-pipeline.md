...
- 1.05 (Phase 1 파이프라인) — 편집·렌더·QA 컴포넌트(03·04·06 조합)를 재사용하고 머리(④ 새 내용 계산)만 `isRetry` 로 분기한다. 05 가 그 컴포넌트 조립을 완료해야 단일 슬라이드 경로를 얹을 수 있다. (콜백 클라이언트 02·컨텍스트 조회는 05 경유로 전이적 확보)
