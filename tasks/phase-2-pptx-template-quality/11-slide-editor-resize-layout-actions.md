...
- [ ] `<a:xfrm>`, `<a:off>`, `<a:ext>` 수정 방식과 기존 좌표 추출 유틸 확인
- [ ] resize 대상 shape 와 linked background fixture 준비

## 구현 체크리스트

- [ ] `SlideEditor.apply_layout_actions()` 메서드 골격 추가
- [ ] `resize_shape` action 으로 shape width/height 를 OOXML 에 반영
- [ ] `resize_linked_shape` action 으로 text shape 와 background shape 크기를 함께 수정
- [ ] 잘못된 shape id/action payload 는 slide-level 오류로 반환 또는 raise
- [ ] `tests/test_features/test_visualization/test_slide_editor.py` 에 resize action 테스트 추가

## Definition of Done

- [ ] 작업 중 또는 완료 후 새 사용자 확인사항이 생기면 `tasks/phase-2-pptx-template-quality/18-user-signoff-and-operational-readiness.md` 에 추가했다.
- [ ] `resize_shape` 적용 후 OOXML ext 값이 변경된다
- [ ] `resize_linked_shape` 적용 후 text box 와 background 폭이 모두 변경된다
- [ ] 기존 `apply_fills()` 의 text/remove/chart 동작은 회귀하지 않는다
