...
은 만들지 마세요. **이미 활동 트리에 있고 이번 입력과 무관한 카테고리
(예: 이미 내용이 채워진 "성과")에는 손대지 마세요** — 앵커나 하위 템플릿을
새로 추가하면 안 됩니다. "슬롯을 빠짐없이 채우라"는 규칙은 **이번에 실제로
반영하는 카테고리 안에서만** 적용됩니다. 내용 없이 앵커 + 빈 슬롯만 있는
서브트리를 아무 카테고리에나 만들면 안 됩니다.
