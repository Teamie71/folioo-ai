...
`TASK`·`PROBLEM_SOLVING`의 앵커(level 4) 블록 하나는 **업무 하나, 문제해결
에피소드 하나**입니다. 한 활동에서 여러 업무를 맡고 여러 문제를 해결할 수
있으므로, **같은 카테고리 컨테이너 아래에 앵커가 여러 개 나란히 있는 것이
정상**입니다. 활동 트리에서 **`〔앵커〕` 표시가 붙은 블록이 기존 앵커**입니다.
앵커의 요약이 비어 있어도("(빈 블록 — 가이드: ...)") 그 아래 세부 슬롯 내용이
곧 그 에피소드의 내용입니다. `〔앵커〕` 표시가 없는 자유 텍스트 블록은 앵커가
아니라 그냥 내용 하나입니다.

이번 입력이 기존 에피소드를 이어서 보강하는지는 앞 단계에서 이미 판정해 아래
사용자 메시지에 **"에피소드 판정"**으로 알려줍니다. 그 판정을 그대로 따릅니다.

- **기존 앵커를 이어서 보강한다고 하면** 그 앵커를 새로 만들지 말고, 그 앵커
  별칭을 그대로 `parent_ref`로 써서 level 5 슬롯들을 바로 답니다. 새 앵커를
  또 만들면 거부됩니다.
- **새 업무·에피소드라고 하면** 기존 앵커 아래에 붙이지 말고 앵커부터 새로
  만듭니다(컨테이너 별칭을 `parent_ref`로, 앵커에 `slot_id` 지정). 기존 앵커가
  몇 개 있든 상관없습니다.
- 판정이 없으면 기존 앵커가 없는 것이므로 평소대로 새 앵커를 만듭니다.
...
내용 없이 앵커만 있는 서브트리를 아무 카테고리에나 만들면 안 됩니다 — 만드는
카테고리마다 실제 원문 내용이 최소 하나는 있어야 합니다.
...
**입력에 서로 다른(별개의) 업무나 문제해결 에피소드가 여러 개 섞여 있으면,
에피소드마다 앵커를 따로 하나씩 만듭니다.** 카테고리 컨테이너는 하나이고, 그
아래에 앵커가 에피소드 수만큼 나란히 옵니다. 각 앵커 아래에는 그 에피소드에
맞는 템플릿 하나를 골라 그 에피소드의 원문만 배정합니다 — 다른 에피소드의
문장을 섞지 않습니다. **절대로 한 슬롯 안에서 문제·원인·해결처럼 서로 다른
단계의 문장을 섞지 마세요** — 슬롯 하나에는 그 슬롯이 맡은 단계의 문장만
들어갑니다. 모든 원문 item은 정확히 한 에피소드의 한 슬롯에만 배정되어야
하며, 어떤 원문도 반영을 건너뛰면 안 됩니다.
...
**기존 카테고리에 새 업무·에피소드를 추가할 때도 이 3단 구조는 그대로
지킵니다.** 카테고리 컨테이너가 이미 트리에 있을 뿐, 새 에피소드의 앵커는
**이번에 새로** 만드는 겁니다 — level 5를 그 기존 컨테이너 별칭에 바로 붙이면
안 됩니다:
...
- **내용과 같은 section의 카테고리가 트리에 이미 있을 때만 재사용합니다.**
  ...
  별칭을 그대로 `parent_ref`로 써서 앵커부터 바로 답니다.
- **내용이 없는 level 5 형제 슬롯은 직접 만들 필요가 없습니다.** 하위
  템플릿을 골랐다면, 실제로 채운 level 5 슬롯만 만드세요 — 원문이 없는
  나머지 형제 슬롯을 빈 item(`text: null`)으로 일일이 만들지 않아도, 코드가
  자동으로 채웁니다.
  **단, 그 템플릿의 앵커(level 4) 블록 자체는 반드시 만듭니다** — 앵커에
  담을 별도 요약 원문이 없어도(`text: null`이어도), level 5 슬롯을 달 부모가
  없으면 안 되므로 앵커 item은 생략하지 마세요. level 5 item은 항상 그
  앵커의 `parent_item_id`를 가리켜야 하고, 카테고리 컨테이너에 직접 붙이면
  안 됩니다.
  section에 하위 템플릿 없이 level 4 슬롯이 여러 개 나란히 있는 경우(예:
  `ACHIEVEMENT`의 `QUANTITATIVE`·`QUALITATIVE`)는 내용이 있는 슬롯만
  만들면 됩니다 — 이 경우는 앵커 없이 슬롯 자체가 카테고리 컨테이너 바로
  아래에 옵니다.
  **어느 경우든 존재하지 않는 slot_id를 지어내거나 다른 템플릿의 slot_id를
  섞어 쓰면 안 됩니다** — 카탈로그에 실제로 있는 slot_id만 씁니다.
