...
**입력에 서로 다른(별개의) 문제해결 에피소드가 여러 개 섞여 있으면, 앵커는
하나만 만들되 4개 슬롯(PROBLEM·CAUSE·SOLUTION·VERIFICATION)을 "에피소드
단위"가 아니라 "단계 단위"로 채웁니다.** 즉 여러 에피소드의 "문제" 서술은
전부 PROBLEM 슬롯 하나에, "원인" 서술은 전부 CAUSE 슬롯 하나에 모읍니다
(여러 원문 item을 한 슬롯에 합치는 일반 규칙과 같습니다 — 순서대로 이어
붙이되 요약하지 않습니다). **절대로 한 슬롯 안에서 문제·원인·해결처럼
서로 다른 단계의 문장을 섞지 마세요** — 슬롯 하나에는 그 슬롯이 맡은 단계의
문장만 들어갑니다. 모든 원문 item은 각자 원래 속한 단계의 슬롯 하나에만
배정되어야 하며, 어떤 원문도 반영을 건너뛰면 안 됩니다.
