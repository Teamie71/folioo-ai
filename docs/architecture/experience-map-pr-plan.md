...
>
> **2026-08-16 현황**: EM-01~EM-22는 통합 브랜치 `feat/experience-map`에
> 머지 완료했습니다. EM-23은 자동 시나리오·로컬 운영 검증까지 완료했고, 메인 서버
> 실연동 검증만 외부 의존으로 남았습니다. 상태 표시는 이 기준의 스냅샷입니다.
...
✅ 는 통합 브랜치 머지 완료입니다. ⏳ 는 AI 서버 구현은 끝났지만 메인 서버 연동
환경에서의 최종 검증이 남았다는 뜻입니다. 최종 `dev` PR 전에는 이 표와 실제
GitHub PR 상태를 다시 대조합니다.
...
> ✅ **`slot_id` 38개 목록이 확정됐습니다** (2026-08-09, 에이전트 문서 3-0).
> 그래도 `slot_id`를 코드 상수로 박지 않고 **EM-03 카탈로그에서 받아온 목록으로만**
> 검증합니다. 문구가 바뀌어도 AI 서버 배포가 필요 없어야 하기 때문입니다.
>
> **level에 따라 형식이 둘입니다.** 4단계는 2-part, 5단계는 3-part이며 점 개수로
> level을 판정할 수 있습니다. level 5는 반드시 앵커 슬롯으로 만든 level 4 아래에
> 붙습니다 — 이 앵커 규칙을 검증에 넣어야 합니다.
...
**현재 상태 (2026-08-16)**: 입력·템플릿·graph 시나리오와 로컬 PostgreSQL,
Swagger/Tailscale, 실제 LLM 호출까지 검증했다. 메인 서버의 운영 DB·`/templates`·
`/commit`·되돌리기 API를 붙여 검증하기 전까지 flag 기본값은 `false`로 유지한다.

**DoD**
- [x] 시나리오 14개 전부 통과 (파일 파서/OCR, 채팅, gap 답변 2분기, 동시 입력,
      카테고리 생성, 템플릿 2종, fallback 2종, 재시도, gap 실패, SSE 단절)
- [x] 로컬 PostgreSQL repository·service 통합 테스트 통과
- [x] `ruff check .` · `ruff format --check .` · 경험정리 테스트 통과
- [ ] 메인 서버 연동 통합 테스트 7개 통과
- [ ] 실제 운영 환경 검증 뒤 `EXPERIENCE_MAP_ENABLED` 기본값을 `true`로 전환

### 최종 dev 통합 PR 전 보정 항목

- `1d3c60a` — Swagger/OpenAPI가 세션 생성은 `X-API-Key`, 세션 후속 경로는
  Bearer 티켓 인증으로 정확히 표시하도록 보정했다. 관련 인프라 테스트 17개가 통과했다.
- 위 보정과 EM-23의 외부 연동 검증 결과를 포함한 뒤 `feat/experience-map → dev`
  최종 PR을 연다. PR 본문에는 기존 이슈를 닫는 `Closes` 행을 포함한다.
...
| ID | 항목 | 주체 | 막는 PR | 완화책 |
| --- | --- | --- | --- | --- |
| 외부-A | 메인 DB migration (통합 문서 9절 1번) | 메인 서버 | EM-23 최종 검증 | 로컬 테스트 DB에 동일 스키마 픽스처를 두고 선행 개발 |
| 외부-B | `POST /commit`·`GET /commit/{id}` | 메인 서버 | EM-23 최종 검증 | API 명세 기준 mock 서버로 계약 테스트 |
| ~~외부-C~~ | ~~**`slot_id` 38개 목록**~~ | ~~기획·메인 서버~~ | ~~EM-15~~ | **해소됨 (2026-08-09)** — 에이전트 문서 3-0 |
| 외부-D | 결과 응답 문구 확정 | 기획 | — (막지 않음) | 변수 분리로 문구만 교체 |
| 외부-E | `GET /templates` | 메인 서버 | EM-23 최종 검증 | 계약 테스트로 선행 |
| 외부-F | 되돌리기 API (9절 21번) | 메인 서버 | EM-23 최종 검증 | API 호출 및 충돌·만료 시나리오 검증 |

외부-C(`slot_id` 목록)는 2026-08-09에 해소됐고 AI 서버 구현은 외부-A~F의 mock·
로컬 스키마로 선행 완료했다. 이제 외부-A·B·E·F와 운영 API 키 계약은 **최종
실연동 검증의 임계 경로**다. 이 항목들이 확인되기 전에는 feature flag를 켜거나
`dev` 통합 PR을 완료로 처리하지 않는다.
