...
| 생성 대상 | 템플릿 |
| --- | --- |
| 3단계 카테고리 | 카테고리 슬롯 (3-2) |
| 담당업무 앵커(`TASK.SUMMARY`) 하위 5단계 | 담당업무 템플릿 기본 1종 (3-3) |
| 문제해결 앵커(`PROBLEM_SOLVING.SUMMARY`) 하위 5단계 | **6종 중 선택** (3-4) |
| 그 외 | 기본 placeholder |

5단계는 앵커 슬롯으로 만든 4단계 아래에만 붙습니다 (3-0). 상세정보·주요성과·
배운 점에는 5단계를 만들지 않습니다.
...
| Output | 제안 문구, gap(있으면), gap 유형, 기준 블록 **별칭** |
...
class GapCandidate(BaseModel):
    gap_type: Literal["extend_block", "new_child_block"]
    anchor_ref: str                 # 별칭. 실제 ID 변환은 AI 서버가 수행
    reason: str | None


class GapOutput(BaseModel):
    gap: GapCandidate | None
    message: str
```

`GapCandidate`(LLM 출력, 별칭 기준)와 `ActiveGap`(저장·전송, 실제 ID 기준)은 다른
모델입니다. LLM에는 실제 block ID를 노출하지 않으므로 두 시점을 한 모델로 표현할
수 없습니다. 변환은 gap 분석 노드가 수행합니다 (5-10).
