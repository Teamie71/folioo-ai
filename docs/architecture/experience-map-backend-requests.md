...
| 4 | AI → 메인 호출용 API 키 **값** 합의 | 중간 | 운영에서 커밋·템플릿 호출 전부 401 |
...
명세 2-1 이 양방향 모두 `AI_SERVICE_API_KEY` 를 쓰도록 적혀 있었는데, AI 서버
구현은 아웃바운드에 `MAIN_BACKEND_API_KEY` 를 씁니다. **명세대로 키 하나만
배포하면 커밋·템플릿 호출이 전부 실패합니다.**

### 어떻게 정리했나

**명세를 코드에 맞췄습니다.** 방향마다 다른 키를 쓰는 쪽이 맞다고 판단했습니다.
