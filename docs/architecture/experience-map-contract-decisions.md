...
### 2-5. 대화 히스토리는 AI가 설계·소유 (2026-09-03 추가 결정)

FE가 API 명세를 다시 검토하다가 "대화 히스토리를 조회하는 기능이 없다"는 걸
발견했습니다. 메인 서버에 문의했더니 "관련된 게 없으면 우리가 DB 설계 +
마이그레이션까지 해두겠다"는 답이 왔고, 이후 논의를 거쳐 **AI가 대화 DB를
직접 설계·소유하는 걸로 최종 정리**했습니다 — 대화 원문을 실제로 다루는
유일한 쪽이 AI이고, 이미 `ai_experience_*` 스키마 쓰기 권한을 갖고 있어 여기
붙이는 게 가장 일관적입니다.

- **신설 테이블**: `ai_experience_message` — 다른 `ai_experience_*` 테이블과
  달리 메인 서버 migration과 대조할 대상이 아닙니다. AI가 처음부터 끝까지
  소유합니다. DDL은 `scripts/experience_map/schema.sql`에 있습니다.
- **저장 범위**: 한 턴당 `user_message` + `ai_response` 텍스트만(블록 ID·gap
  메타데이터 등은 담지 않음). 보존 기간·개수 제한 없음(세션당 전체).
- **신설 API**: `GET /sessions/{session_id}/messages` — 커서 기반 페이징. 이
  저장소에서 커서 페이징을 쓰는 첫 엔드포인트입니다.
- **남은 것**: 운영 DB에 `ai_experience_message`를 실제로 반영하는 절차(누가
  언제 DDL을 돌릴지)는 아직 안 정해졌습니다 — 이 저장소에는 자동 migration
  러너가 없습니다.
