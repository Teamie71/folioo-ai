...
body를 읽기 전에 검증하는 이유는 파일 업로드가 프론트 → AI 직결이라 최대 5MB가
...
**추출 상한** (의도된 정책, QA 2026-09-22 #5-1 — 이전에 문서화 공백이었음):

| 상한 | 값 | 초과 시 동작 |
| --- | --- | --- |
| PDF 페이지 수 | `MAX_PDF_PAGES` (10페이지) | 앞 10페이지만 추출하고, 원문 뒤에 안내 문구(`[페이지가 많아 앞 N페이지만 사용했습니다. 전체 M페이지 중 일부입니다.]`)를 붙입니다. |
| 추출 원문 글자 수 | `MAX_FILE_TEXT_CHARS` (40,000자) | 앞부분만 자르고, 원문 뒤에 안내 문구(`[내용이 길어 앞부분만 사용했습니다.]`)를 붙입니다. |

둘 중 하나라도 잘리면 `file_content_truncated` 플래그가 켜지고, 결과 응답에
"첨부 파일 내용이 많아 일부만 반영됐어요" 안내가 덧붙습니다
(`features/experience_map/nodes/result_response.py`). 비용·처리 시간을 무한정
늘리지 않기 위한 의도된 상한이며, 포트폴리오 PDF가 10페이지를 쉽게 넘길 수
있다는 점은 별도 정책 재검토 대상입니다.
...
개수 최대 1개, 파일당 최대 5MB(2026-09-20 축소).
