...
> [포트폴리오 첨삭] 섹션의 화면 주석.
>
> **2026-08-21 현황**: 통합 브랜치 `feat/correction-v35` 에 **CR-01~CR-07 전부 커밋 완료**.
> CR-05 는 메인 서버 변경 없이 배포 가능한 형태로 구현했습니다 — 스트림은 화면 표시용이고
> 저장은 기존 배치 콜백을 그대로 씁니다. 켜려면 메인의 SSE 프록시 라우트만 있으면 됩니다.
> 메인 백엔드(`Teamie71/folioo-server` dev) 대조 결과는 5·6절에 있습니다 — **백엔드 쪽
> v.3.5 반영은 아직 0건**이고, 상태 전이 불일치 2건이 남아 있습니다.
...
4. **계약이 필요한 변경은 문서를 먼저.** CR-04 에서 전달 경로와 저장 정책을 먼저 정하고
   CR-05 를 구현했습니다. 결과적으로 **메인 서버 의존이 없는 방식**을 골랐기 때문에
   외부 승인을 기다리지 않고 구현까지 마쳤습니다.
...
- [x] `POST /corrections/{id}/pdf-extraction/stream` SSE 엔드포인트 추가
- [x] `PdfExtractionGenerator.extract_stream` — `astream` 으로 토큰을 받아 부분 JSON 파싱
- [x] `ActivityJsonStreamParser` — 완성된 `activities[]` 원소만 떼어 냄
      (문자열 안 중괄호·이스케이프 구분)
- [x] 활동 1건 정규화를 `_normalize_activity` 로 분리해 배치 경로와 규칙 공유
- [x] 활동 개수 상한 도달 시 조기 종료
- [x] 실패 시 `extraction_failed` 이벤트 + 메인 실패 콜백
- [x] `interleave_ping_events` 를 `common/sse/ping.py` 로 이동해 인터뷰와 공유
- [x] 테스트: 순서·중복 스킵·개수 상한·중간 실패·빈 결과·파일 검증 실패

**Definition of Done**

- [x] 활동이 완료될 때마다 이벤트가 1건씩 나가고, 마지막에 완료 이벤트가 정확히 1회 발생
- [x] 기존 배치 콜백 소비자가 깨지지 않음 — 저장 경로를 그대로 재사용
- [x] 실패 시나리오에서 `fail_pdf_extraction` 이 정확히 1회 호출

**감수한 것**

- structured output 대신 프롬프트 + 원소별 검증으로 스키마를 강제합니다.
  기존 배치 엔드포인트는 structured output 을 그대로 쓰므로 영향이 없습니다.
- 중간 실패 시 화면에 보이던 활동이 저장되지 않습니다. 메인의 PDF 추출 결과 수신이
  전부-아니면-전무 구조라, 부분 저장을 하려면 메인 변경이 필요합니다.
